resource "aws_security_group" "cart-service_sg" {
  name   = "cart-service-${var.project}-${var.env}-${var.launch_type}-ecs-sg"
...
module "cart-service_task_definition" {
  ...
  definitions = templatefile("definitions/container_definition.json", {
    ...
    definition_name = "cart-service-${var.project}-${var.env}-${var.launch_type}"
    container_port  = 8013
    host_port       = 8013
  })
}
...
module "cart-service_ecs_service" {
  ...
  name            = "cart-service-${var.project}-${var.env}-${var.launch_type}-ecs-service"
  cluster         = module.ecs_cluster.id
  task_definition = module.cart-service_task_definition.arn
  desired_count   = var.desired_tasks
  launch_type     = var.launch_type
  container_name  = "cart-service-${var.project}-${var.env}-${var.launch_type}"
  container_port  = 8013
  network_config = [
    {
