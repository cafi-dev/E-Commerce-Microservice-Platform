resource "aws_security_group" "product-service_sg" {
  name   = "product-service-${var.project}-${var.env}-${var.launch_type}-ecs-sg"
...
module "product-service_task_definition" {
  ...
  definitions = templatefile("definitions/container_definition.json", {
    ...
    definition_name = "product-service-${var.project}-${var.env}-${var.launch_type}"
    container_port  = 8016
    host_port       = 8016
  })
}
...
module "product-service_ecs_service" {
  ...
  name            = "product-service-${var.project}-${var.env}-${var.launch_type}-ecs-service"
  cluster         = module.ecs_cluster.id
  task_definition = module.product-service_task_definition.arn
  desired_count   = var.desired_tasks
  launch_type     = var.launch_type
  container_name  = "product-service-${var.project}-${var.env}-${var.launch_type}"
  container_port  = 8016
  network_config = [
    {
