resource "aws_security_group" "order-service_sg" {
  name   = "order-service-${var.project}-${var.env}-${var.launch_type}-ecs-sg"
...
module "order-service_task_definition" {
  ...
  definitions = templatefile("definitions/container_definition.json", {
    ...
    definition_name = "order-service-${var.project}-${var.env}-${var.launch_type}"
    container_port  = 8015
    host_port       = 8015
  })
}
...
module "order-service_ecs_service" {
  ...
  name            = "order-service-${var.project}-${var.env}-${var.launch_type}-ecs-service"
  cluster         = module.ecs_cluster.id
  task_definition = module.order-service_task_definition.arn
  desired_count   = var.desired_tasks
  launch_type     = var.launch_type
  container_name  = "order-service-${var.project}-${var.env}-${var.launch_type}"
  container_port  = 8015
  network_config = [
    {
