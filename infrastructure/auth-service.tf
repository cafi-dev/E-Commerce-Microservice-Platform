resource "aws_security_group" "auth-service_sg" {
  name   = "auth-service-${var.project}-${var.env}-${var.launch_type}-ecs-sg"
...
module "auth-service_task_definition" {
  ...
  definitions = templatefile("definitions/container_definition.json", {
    ...
    definition_name = "auth-service-${var.project}-${var.env}-${var.launch_type}"
    container_port  = 8012
    host_port       = 8012
  })
}
...
module "auth-service_ecs_service" {
  ...
  name            = "auth-service-${var.project}-${var.env}-${var.launch_type}-ecs-service"
  cluster         = module.ecs_cluster.id
  task_definition = module.auth-service_task_definition.arn
  desired_count   = var.desired_tasks
  launch_type     = var.launch_type
  container_name  = "auth-service-${var.project}-${var.env}-${var.launch_type}"
  container_port  = 8012
  network_config = [
    {
