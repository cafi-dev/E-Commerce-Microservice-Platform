resource "aws_security_group" "user-service_sg" {
	name        = "user-service-${var.project}-${var.env}-ecs-sg"
...
resource "aws_cloudwatch_log_group" "user-service_lg" {
  name = "user-service_${var.project}_${var.env}"
}
...
module "user-service_task_definition" {
	...
	definitions = templatefile("definitions/container_definition.json", {
	  ...
	  definition_name = "user-service-${var.project}-${var.env}"
	  container_port  = 8017
	  host_port       = 8017
	  log_group       = "user-service_${var.project}_${var.env}"
	  region          = var.region
	  prefix          = "user-service"
  ...
  module "user-service_ecs_service" {
	...
	name            = "user-service-${var.project}-${var.env}-ecs-service"
	cluster         = module.ecs_cluster.id
	task_definition = module.user-service_task_definition.arn
	desired_count   = var.desired_tasks
	launch_type     = var.launch_type
	container_name  = "user-service-${var.project}-${var.env}"
	container_port  = 8017
	network_config  = [
		{
